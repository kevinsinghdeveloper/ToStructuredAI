"""Unified ConnectionRepository — wraps any IRepository, backend-agnostic."""
from typing import List, Optional
from abstractions.IRepository import IRepository
from abstractions.IConnectionRepository import IConnectionRepository
...
class ConnectionRepository(IConnectionRepository):
    ...
    def find_by_status(self, user_id: str, status: str) -> List[dict]:
        conns = self._repo.find_by("user_id", user_id)
        return [c for c in conns if c.get("status") == status]
