"""Unified PipelineRepository — wraps any IRepository, backend-agnostic."""
from typing import List, Optional
from abstractions.IRepository import IRepository
from abstractions.IPipelineRepository import IPipelineRepository
...
class PipelineRepository(IPipelineRepository):
    ...
    def find_by_status(self, user_id: str, status: str) -> List[dict]:
        pipelines = self._repo.find_by("user_id", user_id)
        return [p for p in pipelines if p.get("status") == status]
