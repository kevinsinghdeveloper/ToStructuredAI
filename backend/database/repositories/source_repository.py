"""Unified SourceRepository — wraps any IRepository, backend-agnostic."""
from typing import List, Optional
from abstractions.IRepository import IRepository
from abstractions.ISourceRepository import ISourceRepository
...
class SourceRepository(ISourceRepository):
    ...
    def find_by_connection(self, connection_id: str) -> List[dict]:
        return self._repo.find_by("connection_id", connection_id)

    def find_queryable(self, user_id: str) -> List[dict]:
        sources = self._repo.find_by("user_id", user_id)
        return [s for s in sources if s.get("is_queryable")]
