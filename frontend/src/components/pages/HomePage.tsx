...
import {
  CloudUpload, AccountTree, DataObject, QuestionAnswer,
  AutoAwesome, CheckCircle, Speed, TrendingUp,
  FormatQuote, ArrowForward, PlayArrow,
...
const FEATURES = [
  {
    ...
    desc: 'Drag and drop PDFs, Word docs, and more. Automatic text extraction and intelligent chunking for downstream processing.',
  },
  {
    icon: <AccountTree />,
    title: 'RAG Pipelines',
    desc: 'Build retrieval-augmented generation pipelines with vector search. Connect documents to LLMs for accurate, grounded answers.',
  },
  {
    icon: <DataObject />,
    title: 'Structured Output',
    desc: 'Extract structured data from unstructured documents. Define schemas and get clean JSON output every time.',
  },
  {
    icon: <QuestionAnswer />,
    title: 'Ask Questions',
    desc: 'Query your documents in plain English. Get precise answers with source citations and confidence scores.',
  },
];
...
const STEPS = [
  { num: '01', title: 'Upload your documents', desc: 'Drag and drop files into your workspace. We handle parsing and chunking automatically.' },
  { num: '02', title: 'Configure your pipeline', desc: 'Set up RAG pipelines, define output schemas, and connect your preferred LLM provider.' },
  { num: '03', title: 'Get structured results', desc: 'Ask questions, extract data, and receive structured output grounded in your documents.' },
];

const TESTIMONIALS = [
  {
    quote: 'We replaced hours of manual document review with a single pipeline. The structured output is exactly what our downstream systems need.',
    name: 'Sarah Chen',
    role: 'Head of Engineering',
    company: 'Meridian Digital',
  },
  {
    quote: 'The RAG pipeline accuracy is remarkable. Our team can now query thousands of documents and get precise, cited answers in seconds.',
    name: 'Marcus Rivera',
    role: 'Data Science Lead',
    company: 'Apex Studios',
  },
  {
    quote: 'Setting up document processing used to take weeks. With ToStructured AI, we had a production pipeline running in an afternoon.',
    name: 'Priya Sharma',
    role: 'Founder & CEO',
...
const HomePage: React.FC = () => {
  ...
  const primary = theme.palette.primary.main;   // #7b6df6
  const secondary = theme.palette.secondary.main; // #10b981
  const dark = '#0a1628';
  ...
  return (
    <Box sx={{ bgcolor: dark, minHeight: '100vh', overflowX: 'hidden' }}>
        ...
        <Container maxWidth="lg" sx={{ position: 'relative', zIndex: 1 }}>
          <Grid container spacing={6} alignItems="center">
            <Grid item xs={12} md={7}>
              ...
              >
                ToStructured AI helps teams extract, query, and structure information
                from documents using AI-powered pipelines. Start free. Scale when you grow.
              </Typography>
              ...
              >
                ...
                <Box sx={{
                  ...
                  <Typography variant="body2" sx={{ color: primary }}>
                    RAG + Schema Extraction -- 47 documents
                  </Typography>
                </Box>

                {/* Mock document entries */}
                {[
                  { project: 'Invoice_Q4_2024.pdf', client: 'Extracted', time: '12 fields', color: secondary },
                  { project: 'NDA_Meridian.docx', client: 'Processing', time: '8 fields', color: primary },
                  { project: 'SOW_Lightbridge.pdf', client: 'Queued', time: '--', color: '#f59e0b' },
                ].map((entry, i) => (
                  <Box
                    key={i}
                    sx={{
                  ...
                  >
                    <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5 }}>
                      <Box sx={{ width: 3, height: 28, borderRadius: 1, bgcolor: entry.color }} />
                      <Box>
                        <Typography variant="body2" sx={{ color: '#fff', fontWeight: 500 }}>{entry.project}</Typography>
                        <Typography variant="caption" sx={{ color: 'rgba(255,255,255,0.4)' }}>{entry.client}</Typography>
                      </Box>
                    </Box>
        ...
        <Container maxWidth="lg">
          <Box sx={{ textAlign: 'center', mb: { xs: 6, md: 8 } }}>
            ...
            <Typography variant="body1" sx={{ color: 'rgba(255,255,255,0.5)', maxWidth: 560, mx: 'auto' }}>
              From document upload to structured output, ToStructured AI gives your team the
              tools to extract insights from any document.
            </Typography>
          </Box>

          <Grid container spacing={3}>
            {FEATURES.map((feat, i) => {
              const isLarge = i < 2; // first two cards span more
              return (
                <Grid item xs={12} sm={6} md={isLarge ? 6 : 6} key={i}>
                  <Box
                    sx={{
                      p: { xs: 3, md: 4 },
                      height: '100%',
        ...
        <Container maxWidth="md">
          <Box sx={{ textAlign: 'center', mb: { xs: 6, md: 8 } }}>
            ...
            <Typography variant="body1" sx={{ color: 'rgba(255,255,255,0.5)', maxWidth: 480, mx: 'auto' }}>
              No training required. No complex setup. Three steps and your documents are structured.
            </Typography>
          </Box>
          ...
          <Grid container spacing={6} alignItems="center">
            <Grid item xs={12} md={5}>
              ...
              <Typography variant="body1" sx={{ color: 'rgba(255,255,255,0.5)', lineHeight: 1.7, mb: 3 }}>
                Whether you're a solo developer or a 200-person enterprise, ToStructured AI
                scales with your document processing needs. Here's what every plan includes.
              </Typography>
              <Button
                variant="contained"
                endIcon={<ArrowForward />}
        ...
        <Container maxWidth="md">
          ...
          >
            ...
            <Typography variant="h4" sx={{ color: '#fff', fontWeight: 700, mb: 2 }}>
              Teams process 10x more documents per week
            </Typography>
            <Typography variant="body1" sx={{ color: 'rgba(255,255,255,0.55)', maxWidth: 500, mx: 'auto', lineHeight: 1.7 }}>
              Stop manually reviewing documents. When you automate extraction and structuring,
      ...
      >
        ...
        <Container maxWidth="sm" sx={{ position: 'relative', zIndex: 1 }}>
          ...
          <Typography variant="body1" sx={{ color: 'rgba(255,255,255,0.85)', mb: 4, lineHeight: 1.7 }}>
            Join teams who've stopped manually processing documents and started
            extracting structured data with AI. Free forever for individuals.
          </Typography>
      ...
      {/* Pulse keyframe for pipeline dot */}
      <style>{`
        @keyframes pulse {
          0%, 100% { opacity: 1; }
          50% { opacity: 0.4; }
